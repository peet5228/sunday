...
router.get('/eva',verifyToken,requireRole('ผู้รับการประเมินผล'),async (req,res) => {
    try{
        const token = req.user.id_member
        const [[evaCount]] = await db.query(`select count(*) as total from tb_eva,tb_member where tb_member.id_member='${token}' and tb_eva.id_member=tb_member.id_member`)
        const [[one]] = await db.query(`select count(*) as total from tb_eva,tb_member where tb_member.id_member='${token}' and tb_eva.id_member=tb_member.id_member and status_eva=3`)
        const [[two]] = await db.query(`select count(*) as total from tb_eva,tb_member where tb_member.id_member='${token}' and tb_eva.id_member=tb_member.id_member and status_eva!=3`)
        res.json({
            box : [
                {label: 'แบบประเมินทั้งหมด' , value:evaCount.total || 0},
                {label: 'แบบประเมินที่ไม่สำเร็จ' , value: evaCount.total > 0 ? `${ (two.total*100 / evaCount.total ).toFixed(2) }%` : '00.00' },
